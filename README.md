...
@RamyAydarous notes that this method doesn't start the trim from the previous keyframe, and therefore generates an initial static frame that extends into the next keyframe. Switch the order of `-ss` and `-i` parameters to force trim from previous keyframe. 
